...
`;function F(o){const e=new S(o.apiHost,o.siteId,o.lang),t=o.lang,n=document.createElement("div");n.className="chat-widget-container";const i=n.attachShadow({mode:"open"});i.innerHTML=`
    <style>${q}</style>
  ...
  `,document.body.appendChild(n);const c=i.querySelector(".chat-widget-button"),h=i.querySelector(".chat-widget-window"),P=i.querySelector(".chat-widget-close"),l=i.querySelector(".chat-widget-messages"),z=i.querySelector(".chat-widget-faqs"),d=i.querySelector(".chat-widget-input input"),p=i.querySelector(".chat-widget-send"),u=i.querySelector(".chat-widget-form-overlay");let m=!1,b=[],x=!1;function v(){m=!m,m?(h.classList.add("open"),c.style.display="none",x||E()):(h.classList.remove("open"),c.style.display="block")}c.addEventListener("click",v),P.addEventListener("click",v);async function E(){x=!0,await e.createSession(),$()}async function $(){b=await e.getFaqs(),b.forEach(a=>{const s=document.createElement("button");s.className="chat-faq-btn",s.textContent=a.question,s.addEventListener("click",()=>g(a.question)),z.appendChild(s)})}function f(a){const s=document.createElement("div");s.className=`chat-message ${a.role}`,s.textContent=a.content,l.appendChild(s),l.scrollTop=l.scrollHeight}function j(){const a=document.createElement("div");a.className="chat-widget-loading",a.textContent=r(t,"loading"),a.id="loading-msg",l.appendChild(a),l.scrollTop=l.scrollHeight}function y(){const a=i.getElementById("loading-msg");a&&a.remove()}async function g(a){if(a.trim()){d.value="",p.disabled=!0,f({role:"user",content:a}),j();try{const s=await e.sendMessage(a);y(),f({role:"assistant",content:s.reply}),s.needForm&&N()}catch{y(),f({role:"assistant",content:r(t,"networkError")})}finally{p.disabled=!1,d.focus()}}}p.addEventListener("click",()=>g(d.value)),d.addEventListener("keydown",a=>{a.key==="Enter"&&g(d.value)});function N(){C(u,t,async a=>{await e.submitLead(a),k(),f({role:"assistant",content:r(t,"form.success")})},k),u.classList.add("open")}function k(){u.classList.remove("open")}d.addEventListener("input",()=>{p.disabled=!d.value.trim()})}function L(){const o=document.querySelector("script[data-site-id]");return o?{siteId:o.getAttribute("data-site-id")||"",apiHost:o.getAttribute("data-api-host")||"http://localhost:3001",lang:I()}:(console.error("[ChatWidget] 缺少 data-site-id 属性"),{siteId:"",apiHost:"",lang:"zh-CN"})}(function(){const e=L();e.siteId&&F(e)})()})();
