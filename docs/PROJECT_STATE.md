...
## 2026-07-15 侧栏布局约束与 AI 转人工规则讨论
- 修复后台 `.admin-shell` 高度随右侧站点配置展开而增长的问题：shell 固定视口高度，侧栏和工作区设置独立的高度/最小宽度边界，右侧 `.page-content` 独立滚动。
- 新增 AI 连续失败计数：同一会话内 Dify 兜底回复连续出现 2 次后自动将状态设为 `taken_over`，复用现有 `notifyTransfer` 企微/n8n 通知链路；正常 FAQ 或 AI 回复会清零计数，人工接管状态阻止后续 AI 调用。
