...
[[ -z "$(git status --porcelain)" ]] || fail "服务器工作区存在未提交改动，请先处理后再部署"
git remote get-url origin >/dev/null 2>&1 || fail "未配置 GitHub remote：origin"
